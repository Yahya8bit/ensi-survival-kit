...
const STEPS: Step[] = [
  {
    label: 'Initialisation : π(C)=0, voisins directs de C mis à jour, le reste à +∞.',
    current: null,
  ...
  {
    label: 'Sélection de B (π=18, minimum). Aucune relaxation utile (B→C déjà réglé).',
    current: 'B',
    values: { C: 0, A: 19, B: 18, D: 5, E: 8, F: 10, G: 12 },
    settled: ['C', 'D', 'E', 'F', 'G', 'B'],
    relaxed: [],
  ...
  return (
    <div>
      <svg viewBox="0 0 560 380" style={{ width: '100%', maxWidth: 560, display: 'block', margin: '0 auto' }}>
        <defs>
          <marker id="dv-arrow" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto">
          ...
          return (
            <g key={id}>
              <circle
                ...
                cy={y}
                r={18}
                fill={isCurrent ? '#ffd43b' : isSettled ? '#d3f9d8' : 'var(--ifm-background-surface-color)'}
                stroke="var(--ifm-color-emphasis-600)"
                strokeWidth={2}
              />
              <text x={x} y={y + 5} fontSize="14" textAnchor="middle" fontWeight="bold">
                {id}
              </text>
            </g>
          );
