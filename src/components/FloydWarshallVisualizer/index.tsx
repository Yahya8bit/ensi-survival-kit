...
function buildSteps(): Step[] {
  ...
  let P: PCell[][] = L.map((row, i) => row.map((v) => (v === '∞' ? 0 : IDS[i])));

  const steps: Step[] = [
    {
      label: 'L⁽⁰⁾ = L : matrice initiale des arcs directs (0 sur la diagonale, +∞ si aucun arc direct).',
      k: null,
      L: L.map((row) => [...row]),
      P: P.map((row) => [...row]),
  ...
  for (const k of IDS) {
    ...
    steps.push({
      label:
        changed.size > 0
          ? `k=${k} : passage par ${k} améliore ${[...changed].map((c) => `l(${c.replace('-', ',')})`).join(', ')}.`
...
function matrixToLatex(rows: (Cell | PCell)[][], changed?: Set<string>): string {
  const body = rows
    .map((row, ri) =>
      row
        .map((v, ci) => {
...
function Matrix({ title, rows, changed }: { title: string; rows: (Cell | PCell)[][]; changed?: Set<string> }): JSX.Element {
  const html = useMemo(
    () => katex.renderToString(matrixToLatex(rows, changed), { throwOnError: false, displayMode: true }),
    [rows, changed]
  );
  return (
    <div style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', margin: '0 1.5rem' }}>
...
export default function FloydWarshallVisualizer(): JSX.Element {
  ...
  return (
    <div>
      <svg viewBox="0 0 600 430" style={{ width: '100%', maxWidth: 420, display: 'block', margin: '0 auto' }}>
        <defs>
          <marker id="fw-arrow" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto">
        ...
        {EDGES.map(([a, b, w]) => {
          ...
          return (
            <g key={`${a}-${b}`}>
              <path d={`M ${x1} ${y1} Q ${midx} ${midy} ${x2} ${y2}`} stroke="var(--ifm-color-emphasis-500)" strokeWidth={1.5} fill="none" markerEnd="url(#fw-arrow)" />
              <text x={midx} y={midy} fontSize="12" textAnchor="middle" fill="var(--ifm-font-color-base)">
                {w}
              </text>
            </g>
          );
        ...
        {IDS.map((id) => {
          ...
          return (
            <g key={id}>
              <circle cx={x} cy={y} r={22} fill={isCurrent ? 'var(--ifm-color-primary)' : 'var(--ifm-color-emphasis-100)'} stroke="var(--ifm-color-emphasis-600)" strokeWidth={2} />
              <text x={x} y={y + 5} fontSize="14" textAnchor="middle" fontWeight="bold" fill={isCurrent ? 'white' : 'var(--ifm-font-color-base)'}>
                {id}
              </text>
            </g>
          );
      ...
      <div style={{ overflowX: 'auto', display: 'flex', justifyContent: 'center', flexWrap: 'wrap' }}>
        <Matrix title={`L${step.k === null ? '⁽⁰⁾' : ` (k=${step.k})`}`} rows={step.L} changed={step.changedCells} />
        <Matrix title={`P${step.k === null ? '⁽⁰⁾' : ` (k=${step.k})`}`} rows={step.P} changed={step.changedCells} />
      </div>
      <p style={{ textAlign: 'center', fontSize: '0.85rem', color: 'var(--ifm-color-emphasis-600)' }}>
